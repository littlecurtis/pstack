...
2. Pick the wake mechanism using your harness's `/loop` command (built into Cursor and Claude Code, not a pstack skill. Without one, see **Harness** in SKILL.md). An event to watch (CI, a merge, a ref advancing) gets a watcher subagent that wakes you on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.
