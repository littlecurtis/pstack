...
- **Same suite, same result.** The full suite (`php artisan test --compact`) was run before and after, and both counts are in the hand-back. A test changed in a refactor is a finding unless the change only follows a rename.
...
- **No quiet behavior change.** Different defaults, ordering, error messages, journal actions or response shapes are all behavior. Diff them.
